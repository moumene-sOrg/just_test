...
// app.use(
//   "/User-Dashbord",
//   express.static(path.join(__dirname, "User-Dashbord")),
// );
...
//       if (vetRows && vetRows.postal_code)
//         targetPostalCode = vetRows.postal_code;
//     }
//     let where = {
...
//       include: {
//         users: {
//           select: { first_name: true, last_name: true, postal_code: true },
//         },
...
//   try {
//     try {
//       await prisma.vet_rejections.create({
//         data: {
//           vet_id: parseInt(vet_id, 10),
//           demand_id: parseInt(demandId, 10),
//         },
//       });
//     } catch (e) {}
...
//   const { demand_id, diagnosis, treatment, notes, shelter_placement } =
//     req.body;
//   const vet_id = req.user.id;
...
//       data: {
//         demand_id: parseInt(demand_id, 10),
//         vet_id: parseInt(vet_id, 10),
//         diagnosis,
//         treatment,
//         notes,
//         shelter_placement: shelter_placement || "no",
//       },
//     });
...
//     const isMatch =
//       (await bcrypt.compare(oldPassword, currentPass).catch(() => false)) ||
...
//         where: {
//           refuge_id_demand_id: {
//             refuge_id: req.user.id,
//             demand_id: parseInt(id, 10),
//           },
//         },
//         update: {},
...
//       vet_name: e.vets?.users
//         ? `${e.vets.users.first_name} ${e.vets.users.last_name}`
//         : null,
//     }));
//     res.json(rows);
//   } catch (err) {
...
//       return res
//         .status(400)
//         .json({ error: "demand_id and accepted_by are required" });
//     try {
...
//       res
//         .status(201)
//         .json({ message: "Demand accepted successfully", id: result.id });
//     } catch (err) {
//       res.status(500).json({ error: err.message });
//     }
//   },
...
//       include: {
//         demands: { select: { id: true, animal_type: true, created_at: true } },
//       },
//     });
...
//     res
//       .status(201)
//       .json({ message: "Report created successfully", id: result.id });
//   } catch (err) {
//     res.status(500).json({ error: err.message });
//   }
// });
...
//       status:
//         e.demands?.demands_assignments?.length > 0 ? "accepted" : "completed",
//       demands: undefined,
//       vets: undefined,
//     }));
//     res.json(reports);
...
//     });
//     if (!rowRaw || !rowRaw.refuges)
//       return res.status(404).json({ message: "Refuge not found" });
...
//       opening_till: closeTime
//         ? closeTime.toISOString().substring(11, 16)
//         : null,
//     });
//   } catch (err) {
//     res.status(500).json({ error: err.message });
...
//   const { name, email, add, maxcap, cap, open, close, password, oldPassword } =
//     req.body;
//   const user_id = req.user.id;
//   try {
//     if (password) {
...
io.on("connection", (socket) => {
  ...
  socket.on("send-message", (data) => {
    ...
    socket.to(data.to).emit("receive-message", payload);
  });
